-- Vistas base para consumir tiempo usado real de ActivityTimeline.

DROP VIEW IF EXISTS AT_WORKLOGS_RESUMEN_DIARIO;
CREATE VIEW AT_WORKLOGS_RESUMEN_DIARIO AS
SELECT
    username,
    issue_key,
    project_key,
    fecha,
    ROUND(SUM(horas_usadas), 2) AS horas_usadas,
    SUM(time_spent_seconds)     AS time_spent_seconds,
    COUNT(*)                    AS cantidad_worklogs
FROM at_worklogs
GROUP BY username, issue_key, project_key, fecha;
...
FROM at_eventos e
LEFT JOIN AT_WORKLOGS_RESUMEN_DIARIO w
...
DROP VIEW IF EXISTS RPT_AT_HORAS_USADAS_PERSONA_TIPO;
CREATE VIEW RPT_AT_HORAS_USADAS_PERSONA_TIPO AS
SELECT
    v.username,
